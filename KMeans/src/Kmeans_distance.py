# -*- coding: utf-8 -*-
# @Time    : 2019/6/27 21:24
# @Author  : Weiyang
# @File    : Kmeans_distance.py

########################################################################################################################
# 基于点与点之间的欧式距离实现KMeans聚类算法

# 算法思想：
# 首先选择K个中心点，然后将每个样本点分配到距离其最近的中心点，之后更新每个簇的中心点；
...
#       计算所有样本点之间的距离，选择距离最大的一个点对(两个样本a,b),从样本点集中删除这两个点；
#       从剩余点中计算每个点距离已知聚类中心点的最近距离(比如某个点与a最近，某个点与b最近)，然后排序距离，选择距离最远的那个点；
...
#          SSE是误差平方和，它是所有样本点到它所在聚类中心点的距离之和；
# 3. 距离的度量：曼哈顿距离(CityBlock Distance,也叫城市街区距离)，欧式距离(Euclidean Distance)，闵式距离(Minkowski Distance)
#    当 lambda=1时，闵式距离就是曼哈顿距离；当lambda=2时，闵式距离就是欧式距离；
########################################################################################################################
...
class KMeans:
    '''KMeans聚类算法'''
    def __init__(self,n_clusters,init='k-means++',distance='E',max_iter=1000):
    ...
    def getDistance(self,vec1,vec2):
        '''获取两个向量之间的距离'''
        diff = [x-y for x,y in zip(vec1,vec2)] # 相应维度的差值
        if self.distance == 'C':
            distances = sum(map(lambda x:abs(x),diff))
        elif self.distance == 'E':
            distances = np.sqrt(sum(map(lambda x: x ** 2, diff)))
        return distances
    ...
    def getPointsDistance(self,points1,points2):
        ...
        if self.distance == 'E':
            ...
            for point_a in A :
                temp = [] # 存储距离
                for point_b in B:
                    diff = [x-y for x,y in zip(point_a,point_b)]
                    dis = sum(map(lambda x:abs(x),diff))
                    temp.append(dis)
                CD.append(temp)
            CD = np.array(CD) # 曼哈顿距离
            return CD
    ...
    def selectCenterPoints(self,points):
        ...
        if self.init == 'random':
            ...
            # 首先选取距离最大的一个点对
            # 计算points中每两个点之间的距离
            distance = self.getPointsDistance(points,points)
            # 最大距离的索引
            ind_max = np.unravel_index(np.argmax(distance),distance.shape)
            centers = [] # 存储聚类中心
            ...
            while True:
                ...
                index = None # 存储距离最大点的索引
                dis = 0 # 存储最大距离
                for point in remaining_points:
                    # 当前点与聚类中心点的最近距离
                    temp = min([distance[point,i] for i in centers_index])
                    if temp > dis :
                        dis = temp
                        index = point
                # 将当前点加入到聚类中心
    ...
    def cal_SSE(self,clusters,centers):
        '''计算每个样本点到它所在聚类中心的距离之和,clusters是聚类簇点集，center是相应的聚类中心点集'''
        SSE = 0
        # 遍历类簇
        for label in clusters.keys():
            ...
            # 计算当前类簇中每个样本点与聚类中心的误差平方和
            distance = self.getPointsDistance(points,center)
            # 求和累加
            SSE += np.sum(distance)
        return SSE
    ...
    def fit_predict(self,points):
        ...
        while True:
            ...
            for index in range(len(points)):
                label = np.argmin(distance[index])
                # 记录当前样本点的类别
                labels.append(label)
                # 将当前样本点加入到对应的簇中
    ...
    def predict(self,points):
        ...
        labels = [] # 每个样本点的类别
        # 计算每个样本点与所有聚类中心的距离
        distance = self.getPointsDistance(points,self.cluster)
        # 遍历每个样本点，获取其最近的聚类中心的距离
        for index in range(len(points)):
            # 获取当前样本点的类别
            label = np.argmin(distance[index])
            # 记录当前样本点的类别
