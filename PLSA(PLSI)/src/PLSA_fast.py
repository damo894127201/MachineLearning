...
class PLSA(object):
    ...
    def fit(self,X,n_topics=5,max_iters=300,threshold=1e-10,init='random'):
        '''概率潜在语义分析
        X 是单词-文本共现数据T,pd.DataFrame结构,维度是(单词，文档)，即(M,N)
        n_topics 人为设定的主题的个数
        max_iters 最大的迭代次数
        threshold 前后两次参数的差值，小于该阈值，则停止迭代
        init 表示初始化条件概率分布P(w_i|z_k) 和条件概率分布P(z_k|d_j) 的方式，取值为 'random' or 'fixed
        ...
        if init == 'fixed':
            ...
            # 随机初始化
            topic_generate_word_matrix = np.random.rand(n_topics * len(self.wordDict)).reshape(n_topics,len(self.wordDict))
            topic_generate_word_matrix /= np.sum(topic_generate_word_matrix,axis=1)[:,None]

        # 转为pd.DataFrame
        topic_generate_word_matrix = pd.DataFrame(topic_generate_word_matrix,
                                                  index=['Topic:' + str(i) for i in range(n_topics)],
        ...
        if init == 'fixed':
            ...
            # 随机初始化
            doc_generate_topic_matrix = np.random.rand(self.num_Documents * n_topics).reshape(self.num_Documents,n_topics)
            doc_generate_topic_matrix /= np.sum(doc_generate_topic_matrix,axis=1)[:,None]

        # 转为pd.DataFrame
        doc_generate_topic_matrix = pd.DataFrame(doc_generate_topic_matrix,
                                                 index=['Doc: ' + str(id) for id in range(self.num_Documents)],
                                                 columns=['Topic:' + str(i) for i in range(n_topics)])

        # Q函数 P(z_k|w_i,d_j)

        # 存储所有主题topic的P(w_i|z_k)P(z_k|d_j)的矩阵，用字典来存储，因此这是一个三维的矩阵，维度分别是(主题，文档，单词)
        # 第一维用字典来存储，即key是主题，value是 当topic固定时，存储的P(w_i|z_k)P(z_k|d_j)矩阵，
        # 每一行对应一个文档，每列对应一个单词，维度是(文档，单词)
        doc_generate_all_topic_generate_word = {'Topic:' + str(i):None for i in range(n_topics)}

        # 存储P(z_k|w_i,d_j)的一个三维矩阵，维度分别是(主题，文档，单词)，第一维用字典来存储，即key是主题，
        # value是 当topic固定时，存储的P(z_k|w_i,d_j)矩阵，每一行对应一个文档，每列对应一个单词，维度是(文档，单词)
        word_and_doc_generate_topic_matrix = {'Topic:' + str(i):None for i in range(n_topics)}
        ...
        for step in range(1,max_iters+1):
            ...
            # 存储概率之和，即P(z_k|w_i,d_j)的分母
            sum_value = np.zeros((self.num_Documents,len(self.wordDict))) # (N,M)
            # 先遍历每个主题
            for topic in Topics:
                # 当topic固定时，存储的P(w_i|z_k)P(z_k|d_j)的矩阵，每一行对应一个文档，每列对应一个单词,维度是(N,M)
                temp1 = doc_generate_topic_matrix[topic].values[:,np.newaxis] # 最后一维增加一维
                temp2 = topic_generate_word_matrix.loc[topic].values[np.newaxis,:] # 第一维增加一维
                doc_generate_topicK_generate_word = temp1.dot(temp2)
                sum_value += doc_generate_topicK_generate_word
                # 转为pd.DataFrame，维度是(文档，单词)，即(N,M)
                doc_generate_topicK_generate_word = pd.DataFrame(doc_generate_topicK_generate_word,
                                                                 index=['Doc: ' + str(id) for id in range(self.num_Documents)],
                                                                 columns=self.wordDict)
                doc_generate_all_topic_generate_word[topic] = doc_generate_topicK_generate_word.copy()
            # 归一化概率
            for topic in Topics:
                # 更新Q函数
                word_and_doc_generate_topic_matrix[topic] = doc_generate_all_topic_generate_word[topic] / sum_value
            print(word_and_doc_generate_topic_matrix[topic])
            print(sum_value.shape)

            # M步：求Q函数的极大，更新参数
            # 先遍历每个主题
            for topic in Topics:
                # 计算n(w_i,d_j)*P(z_k|w_i,d_j)的矩阵，维度是(文档，单词)，即(N,M)
                matrix = (word_and_doc_generate_topic_matrix[topic].values) * (X.T.values)
                # 更新参数
                topic_generate_word_matrix.loc[topic] = np.sum(matrix,axis=0) / np.sum(np.sum(matrix,axis=0))
                # 更新参数
                doc_generate_topic_matrix[topic] = np.sum(matrix,axis=1) / np.sum(X,axis=0)
...
if __name__ == '__main__':
    ...
    print(W)
    print(H)
    print(np.sum(W.values,axis=1))
    print(np.sum(H.values,axis=1))