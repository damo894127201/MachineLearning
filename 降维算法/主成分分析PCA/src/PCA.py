...
#=======================================================================================================================
# 主成分分析PCA：基于协方差矩阵的特征值(Eigendecomposition)分解算法
...
# 1. 对样本数据X的每个维度的数据进行规范化处理，即每个维度的值减去该维度的均值再除以标准差，
#    得到中心化的样本Y,新样本每个维度均值为0，方差为1；
...
# 这里需要特别注意的是 样本数据矩阵X 的每一列代表一条数据，即输入 X_m*n 矩阵，输出降维后的Y_k*n矩阵，再转置一下便是我们习惯的
# 矩阵表述方式，每行代表一条数据

# 但是下面的实现中，X的每一行代表的是一条数据，具体实现与上述有些差别。
#=======================================================================================================================
import numpy as np
...
class PCA:
    ...
    def fit(self,X):
        ...
        # 矩阵特征值分解：特征值 ，特征向量
        features,feature_vectors = np.linalg.eig(cov_matrix) # feature_vectors的每一列是一个特征向量
